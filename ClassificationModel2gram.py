...
class RandomIndexing(object):
    ...
    def vocab_exists(self):
        return os.path.exists('vocab2.txt')   
    
    def vectors_exist(self):
        return os.path.exists('ri_vectors2.txt')
    ...
    def create_word_vectors(self):
        ...
        for sentence in self.sentences:
            ...
            for ind, w in enumerate(sent):
                if ind+2 <= len(sent):
                    k1 = ''
                    for ik, k in enumerate(sent[ind:ind+2]):
                        if ik < len(sent[ind:ind+2])-1:
                            k1 += k + " "
                        else:
                            k1 += k
                    words.append(k1)
    ...
    def write_vectors(self):
        c = 0
        with open('ri_vectors2.txt', 'w', errors ='ignore') as f:
    ...
    def read_word_vectors(self):
        ...
        if vectors_exist:
            with open('ri_vectors2.txt') as f:
                for line in f:
                    vals = line.split()
                    word = vals[0]+" "+vals[1]
                    cv[word] = np.array(list(map(float, vals[2:])))
        self.__cv = cv
...
if __name__ == '__main__':
    ...
    train_ds = Dataset('data/X_words/training_X2_no_punctuation', 'data/Y_vectors/training_Y2_no_punctuation')
    train_ds.readFiles()
    test_ds = Dataset('data/X_words/testing_X2_no_punctuation', 'data/Y_vectors/testing_Y2_no_punctuation')
    test_ds.readFiles()
    ...
    if vector_flag == 'ri':
        ...
        ri = ManualIndexing('data/X_feature_vectors_v2/training_X2_v2')
        x = ri.read_vectors()
        y = train_ds.y
        mod_name = 'b2_manual.model'
    print(x[0])    
    ...
    if vector_flag == 'ri':
        ...
        ri = ManualIndexing('data/X_feature_vectors_v2/testing_X2_v2')
