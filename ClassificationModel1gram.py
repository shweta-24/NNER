...
class RandomIndexing(object):
    ...
    def vectors_exist(self):
        return os.path.exists('ri_vectors.txt')
    ...
    def write_vectors(self):
        c = 0
        with open('ri_vectors.txt', 'w', errors ='ignore') as f:
    ...
    def read_word_vectors(self):
        ...
        if vectors_exist:
            with open('ri_vectors.txt') as f:
                for line in f:
                    vals = line.split()
                    if vals[0] == 'and':
                        print('Adding vector for and')
                    cv[vals[0].strip()] = np.array(list(map(float, vals[1:])))
        self.__cv = cv
...
if __name__ == '__main__':
    ...
    train_ds = Dataset('data/X_words/training_X1_no_punctuation', 'data/Y_vectors/training_Y1_no_punctuation')
    train_ds.readFiles()
    test_ds = Dataset('data/X_words/testing_X1_no_punctuation', 'data/Y_vectors/testing_Y1_no_punctuation')
    test_ds.readFiles()
    ...
    if vector_flag == 'ri':
        ...
        ri = ManualIndexing('data/X_feature_vectors_v2/training_X1_v2')
        x = ri.read_vectors()
        y = train_ds.y
        mod_name = 'b_manual.model'
    print(x[0])    
    ...
    if vector_flag == 'ri':
        ...
        mi = ManualIndexing('data/X_feature_vectors_v2/testing_X1_v2')
